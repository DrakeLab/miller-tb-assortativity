...
```{r networkStructures, fig.cap="Simulated networks were generated and then re-wired to varying levels of assortativity (A). The algorithm used to re-wire networks accepted assortativity levels within a range (0.035) and these assortativity values are linearly related to modularity (B). As assortativity increased, clustering (C), path length (D), and degree assortativity (E) were altered, sometimes differently for scale-free and small-world networks. ", fig.width=5, fig.height=8}
grid.arrange(g1, 
             g2, 
             heights=c(1,3))
```
...
res %>%
  filter(modName == "SLIR", tau == 0.075, r%in%c(0, 0.3, 0.9), alph_val%in%c(1, 2, 3)) %>%
...
res %>%
  filter(modName == "SLIR", tau == 0.15, r%in%c(0, 0.3, 0.9), alph_val%in%c(1, 2, 3)) %>%
  ggplot(aes(x=as.factor(alph_type), y=mf_r_ratio, fill=factor(r))) +
  geom_boxplot() + facet_grid(type_net ~ alph_val) +
  geom_hline(yintercept=1.8, linetype="dashed", 
                color = "red", size=.75) +    
...
res %>%
  filter(modName == "SLIRS", tau == 0.075, r%in%c(0, 0.3, 0.9), alph_val%in%c(1, 2, 3)) %>%
...
res %>%
  filter(modName == "SLIRS", tau == 0.15, r%in%c(0, 0.3, 0.9), alph_val%in%c(1, 2, 3)) %>%
  ggplot(aes(x=as.factor(alph_type), y=mf_i_ratio, fill=factor(r))) +
  geom_boxplot() + facet_grid(type_net ~ alph_val) +
  geom_hline(yintercept=1.8, linetype="dashed", 
                color = "red", size=.75) +    
