...
4. Repeat step 3 until $|r_f - r_t|\leq \epsilon$ or until a max number of re-wirings is completed. 
...
for(i in 1:5000){
  ...
  # matrix of edges c(edge_id,from_id, to_id)
...
Gg <- simplify(Gg) # remove repeated edges and self edges
...
This test algorithm shows a major issue: It produces self and multiple loops and removing them lowers the mean degree pretty substantially (at least with graph size of 100). 

igraph has a function to re-wire edges that prevents self and multiple loops but we need to modify the use slightly so that it's only re-wiring edges that are between sexes. 
...
# network generation parameters
alph <- 0.05 # re-wire % of between edges at once
eps <- 0.05  # tolerance level for assortativity
r_f <- 0.3
...
for(i in 1:5000){
  ...
  # Step 3b: select all between edges for potential rewiring
  eb <- which(eTypes=="b")
  GgSub <- subgraph.edges(Gg, eids=eb)

  # Step 3c: re-wire subset of between sex edges and replace old edges
  GgSub <- get.edgelist(rewire(GgSub, each_edge(prob = alph)))
  el[eb, ] <- GgSub
  
  # Step 3d: recreate entire graph and calculate assortativity
  Gg1 <- graph_from_edgelist(el, directed=FALSE)
  V(Gg1)$sex <- sexes$sex
  rt1 <- assortativity_nominal(Gg1, types=V(Gg1)$sex) # basically 0
  
  # Step 3e: if new assortativity is closer to rf, keep new graph
  if(rt1 < rt) next # less assortative than before 
  if(rt1 > (r_f + eps)) next  # way too assortative skip
...
assortativity_degree(Gg0)
assortativity_degree(Gg)

```

Notes: 

* need to generalize for neg assort. coefficient (could base iteration progress on whether $r_t$ is getting closer to $r_f$) rather than if $r_t$ is getting bigger. This would also clean up one level of the if statements. 
* also should begin to structure this code into a few nice functions rather than sad for loop. 
