...
def process_file(f): # parallelized version of Sah et al. (PNAS 2019) network generation algorithm
	...
	nx.write_graphml(G, "networks0/Sah_Q"+str(q)+"_N"+str(n)+"_rep"+str(y)+".graphml")
...
Q= [0] #[0, 0.05, 0.15, 0.25, 0.35, 0.45]                    # network modularity 
Y= range(1000,1200)

var_grid = list(ParameterGrid({'N' : N, 'Q' : Q, 'Y' : Y}))

##### Create and save networks  ########
p = multiprocessing.Pool(25) # create a pool of workers
