...
```{r netPlots}
# SF network plots
n0 <- read_graph("simulations-rewiring/networks4/G_0N100rep1.graphml", format="graphml")
n3 <- read_graph("simulations-rewiring/networks4/G_0.3N100rep1.graphml", format="graphml")
n6 <- read_graph("simulations-rewiring/networks4/G_0.6N100rep1.graphml", format="graphml")
n9 <- read_graph("simulations-rewiring/networks4/G_0.9N100rep1.graphml", format="graphml")
...
tmp <- read.csv("simulations-rewiring/SLIRS-res/slirs-prelim4.csv")

tmp %>%
  mutate(stat="Modularity") %>%
  ggplot(aes(r, q, color=factor(r))) + 
  geom_abline(slope = .5) +
...
```{r networkStructures, fig.cap="Simulated networks were generated with the Sah or rewiring algorithm to varying levels of assortativity (A). Assortativity values are linearly related to modularity when there are two modules, i.e., sexes (B). As assortativity increased, clustering (C), path length (D), and degree assortativity (E) were altered depending on the algorithm used to generate assortative networks. ", fig.width=5, fig.height=8}
grid.arrange(g1, 
             g2, 
             heights=c(1,3))
```
...
res <- read.csv("simulations-rewiring/SLIRS-res/full_res.csv")
