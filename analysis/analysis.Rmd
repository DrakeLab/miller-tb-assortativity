...
tmp %>%
  ...
  ggplot(aes(factor(r), q, color=factor(r))) + 
  geom_abline(slope = .5) +
...
sah_res <- readRDS("simulations/SLIRS-res/sah_res_300.RDS") %>%
  filter(failed=="no")
sah_res$net_type="SAH"

sah_res$modNameLatent <- ifelse(sah_res$delt==0.1, T, F)
sah_res$modNameRecover <- ifelse(sah_res$psi==0.33, T, F)
sah_res$modName <- NA
sah_res$modName[!sah_res$modNameLatent&!sah_res$modNameRecover] <- "SIR"
sah_res$modName[sah_res$modNameLatent&!sah_res$modNameRecover] <- "SLIR"
sah_res$modName[!sah_res$modNameLatent&sah_res$modNameRecover] <- "SIRS"
sah_res$modName[sah_res$modNameLatent&sah_res$modNameRecover] <- "SLIRS"

sah_res$alph_type <- factor(sah_res$alph_type, labels = c("IP", "SUS", "TRA"))

sah_res$mf_r_ratio <- sah_res$m_rec/sah_res$f_rec
sah_res$mf_i_ratio <- sah_res$m_inf/sah_res$f_inf
sah_res$infected_prev <- sah_res$m_inf+sah_res$f_inf
sah_res$recovered_prev <- sah_res$m_rec+sah_res$f_rec



sah_res %>%
  select(r, net_clus, net_path_len, net_deg_assort, net_type) %>%
  gather("stat", "val", -r, -net_type) %>%
  rename(Q=r) %>%
  ggplot(aes(factor(Q), val, color=factor(Q))) + 
  geom_boxplot(alpha=.5) +  
  facet_grid(stat~., scales="free_y") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
        legend.position = "none")  +
  scale_color_discrete_sequential(palette="Sunset") +
  labs(x="Modularity  (Q)", y="", color="Modularity", title = "B") +
  guides(colour=FALSE) +
  theme_bw() +
  theme(plot.title = element_text(face="bold", size=10), 
        axis.text=element_text(size=10),
...
res %<>% rename(type_net=net_type)

res <- full_join(res, resRewired)
