...
knitr::opts_chunk$set(fig.path = "analysis-figures/", 
...
## SIR spread on assorted networks (old re-wiring method): 
...
# taus=c(0.02, 0.05, 0.08, 0.16, 0.24)
# r0=c(0.5,1, 1.5,3,4.5,6)
...
## SIR spread on assorted networks with narrowed parameter range and increased replicates (new re-wiring method):

```{r sirTrajectories2, fig.height=6, fig.width=6, fig.cap="Simulated outbreak trajectories (50 replicates shown for each parameter combination) for SIR model. Columns faceted by R0 and rows faceted both by alpha (i.e., ratio of male:female susceptibility) and assortativity (r)."}

## Repeat analysis with 300 simulations
res3 <- read.csv("simulations-rewiring/SIR/variable_results_3.csv")
res3$roundR <- round(res3$r, 1)

pvars <- expand.grid(reps=1:50, tau=c(0.05, 0.08, 0.16), alph=c(1,"2.0"), rs=c(0, .3, .6, .9))
...
for(i in 1:nrow(pvars)){
  ...
  sim <- read.csv(paste0("simulations-rewiring/SIR/var-suscept-3/SIR_R", foo$rs, 
                         "_N", 1000, 
                         "_tau", foo$tau, "_alph", foo$alph,
                         "_rep", foo$reps, ".csv"))
  sim$r0 <- get_r0(foo$tau, read_graph("simulations-rewiring/networks3/G_0N1000rep1.graphml", format="graphml"))
  sim$alph <- foo$alph
  sim$rep <- foo$reps
  sim$rs <- foo$rs
...
```{r sirHist, fig.height=6, fig.width=6, fig.cap="Distribution of outbreak sizes (i.e., number of nodes that were infected at the end of 200 time steps) across assortativity (rows) and R0 (columns) values. R0 estimated for SIR model (TBD for SIRS model). Simulations with disease fadeout were excluded for the rest of analysis (fadeout defined as less than 5 % of nodes becoming infected, red dashed line). "}


G <- read_graph("simulations-rewiring/networks3/G_0N1000rep1.graphml", format="graphml")

res3$r0 <- get_r0(res3$tau, G)
...
```{r lineSus2, fig.cap="Male-bias across narrowed parameter range with 300 simulations. Plots faceted by R0. Points and colors show susceptibility ratio between men and women. Networks have 1,000 nodes (fadeout simulations excluded). ", fig.height=6, fig.width=6}

outs <- res3 %>%  # filter out outbreaks
  filter(tot_inf > 0.05) 
...
```{r size2, fig.height=6, fig.width=6, fig.cap="Total proportion of infected nodes across assortativity and susceptibility ratio values. Columns faceted by susceptibility. Rows faceted by estimated R0 (300 replicates, fadeouts excluded)."}
...
## SIRS pathogen spread on assorted networks

Simulations start with 5% infected (equal for both male and female nodes). SIRS simulations run for 200 steps. 

```{r sirsTrajectories, fig.height=6, fig.width=6, fig.cap="Simulated SIRS trajectories (5 replicates shown for each parameter combination) for SIRS model. Columns faceted by tau (R0 TBD) and rows faceted both by alpha (i.e., ratio of male:female susceptibility) and assortativity (r)."}

res4 <- read.csv("simulations-rewiring/SIRS/results.csv")
res4$roundR <- round(res4$r, 1)

res4$r0 <- get_r0(res4$tau, G)

pvars <- expand.grid(reps=1:5, tau=c(0.12, 0.24, 0.36), alph=c("1.0","2.0"), rs=c(0, .3, .6, .9))
...
for(i in 1:nrow(pvars)){
  ...
  sim <- read.csv(paste0("simulations-rewiring/SIRS/SIRS1_R", foo$rs, 
                         "_N", 1000, 
                         "_tau", foo$tau, "_alph", foo$alph,
                         "_rep", foo$reps, ".csv"))
  sim$tau <- foo$tau
...
simDf %>%
  ...
  geom_line() + 
  facet_grid(factor(rs) ~ alph ~ tau) + 
  labs(x="Time", y="Infecteds") + 
  guides(color=FALSE)

```

```{r sirsSex, fig.height=6, fig.width=6, fig.cap="Simulated SIRS model (1 replicate shown). Columns faceted by tau (R0 TBD) and rows faceted both by alpha (i.e., ratio of male:female susceptibility) and assortativity (r). Only final 100 steps shown. "}
...
```{r sirsHist, fig.height=6, fig.width=6, fig.cap="Distribution of outbreak sizes (i.e., number of nodes that were infected at the end of 200 time steps) across assortativity (rows) and R0 (columns) values. R0 estimated for SIR model (TBD for SIRS model). Simulations with disease fadeout were excluded for the rest of analysis (fadeout defined as less than 5 % of nodes becoming infected, red dashed line). "}
...
```{r lineSusSIRS, fig.cap="Male-bias for SIRS model with 300 simulations. Plots faceted by tau. Points and colors show susceptibility ratio between men and women. Networks have 1,000 nodes (fadeout simulations excluded). ", fig.height=6, fig.width=6}

res4 %>%
  filter(tau %in% c(0.12, 0.24, 0.36), alph < 4) %>%
  group_by(tau, r, alph) %>%
  summarise(mean_prev_ratio=mean(prev_ratio, na.rm=TRUE), se=(sd(prev_ratio, na.rm=TRUE)/sqrt(n))) %>%
  ggplot(aes(r, mean_prev_ratio, color=factor(alph))) + 
  geom_point(size=.75) + 
...
For the SIR model, realistic levels of sex-assortativity, where same-sex social contacts are more common than within-sex social contact, are insufficient to explain observed levels of male-bias (1.9 +- 0.6). Only very high levels of assortativity (r > 0.8) and low transmissibility ($R_0=1.5$) results in a mean prevalence ratio exceeding 1.9. At realistic levels of assortativity (observed in Uganda), $0.1 < r < 0.3$ and low transmissibility, resulting sex-bias varied widely but rarely exceeded 1.5. There are many caveats to these results but at face value, it seems like sex-assortativity by itself cannot explain sex-bias using SIR models for TB. 
