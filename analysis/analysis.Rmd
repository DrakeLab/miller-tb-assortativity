...
```{r rewired_net_plots, fig.height=3, fig.cap="Plots of networks generated with re-wiring algorithm. Node color represents different sexes (modules).  "}
# SF network plots
n0 <- read_graph("rewired-networks/G_0N100rep1.graphml", format="graphml")
n3 <- read_graph("rewired-networks/G_0.3N100rep1.graphml", format="graphml")
n6 <- read_graph("rewired-networks/G_0.6N100rep1.graphml", format="graphml")
n9 <- read_graph("rewired-networks/G_0.9N100rep1.graphml", format="graphml")
...
netData <- read_rds("rewired-networks/netData.rds")
...
sf_res <- read.csv("SLIRS-res/sf_res_250_full.csv")
sw_res <- read.csv("SLIRS-res/sw_res_250_full.csv")
...
# load sah results
sahRes <- read.csv("SLIRS-res/sah_res_250_full.csv") %>% 
...
res$alph_type <- factor(res$alph_type, levels = c("SUS", "TRA", "IP"))
...
resTau <- read.csv("SLIRS-res/sah_res_tau_25.csv")
...
res %>% 
  filter(mod_name=="SLIRS", !is.infinite(tot_lat), type_net=="SAH Geometric",
         r==0, alph_val==1) %>%
  ggplot(aes(y=tot_lat/1000, x=as.factor(tau))) +
  geom_boxplot() + 
  scale_color_discrete_sequential(palette = "Burg") +
  labs(y="Equilibrium latent", x=expression(tau)) +
  theme_bw() +
  theme(axis.text=element_text(size=txs),
        axis.title=element_text(size=txs,face="bold"),
        legend.title = element_text(size=txs),
...
```{r assort_tau_sah_slir, fig.height=12, fig.width=10}

# comparison of mean male bias values for text supporting claim about transmission rate & interaction of assortativity
res %>%
  filter(mod_name=="SLIR", type_net=="SAH Geometric", r%in%c(0, 0.6), alph_val==2) %>%
  select(r, tau, alph_val, alph_type, mf_i_ratio, mf_r_ratio) %>%
  gather("stat", "val",5:6) %>%
  group_by(alph_val, tau, r, alph_type) %>%
  summarise(med_ratio=median(val, na.rm=TRUE)) %>% 
  spread(alph_type, med_ratio)%>%
  kable(digits = 2) 
...
res %>%
  ...
  geom_hline(yintercept=1, linetype="dashed", 
                color = "grey", size=.5)   +
  scale_y_continuous(trans="log10", limits = c(.7, 5)) + 
  scale_fill_discrete_sequential(palette="Sunset") +
  labs(x="Strength of sex trait", y="M:F case bias", 
...
res %>%
  ...
  ggplot(aes(y=factor(alph_val), x=factor(r),  z=mean_ratio)) +
...
res %>%
  filter(mod_name=="SLIR", type_net=="SAH Geometric", 
         alph_val%in%c(1,2,3), tau==0.1, r%in%c(0, 0.2, 0.4, 0.6)) %>%
  rowwise() %>%
  mutate(totR=sum(m_rec, f_rec)) %>%
  select(alph_val, alph_type, peak, totR, sim_dur, r) %>%
  gather("stat", "val", 3:5) %>%
  mutate(stat=factor(stat, levels=c("peak", "totR", "sim_dur"), 
                     labels=c("Peak Size", "Final Size", "Duration"))) %>%
  ggplot(aes(x=factor(alph_val), y=val, fill=factor(r))) + 
  geom_boxplot() + 
  facet_grid(stat~alph_type, scales="free_y") +
  scale_fill_discrete_sequential(palette="Sunset") +
  labs(x="Strength of sex trait", y="", fill="Assortativity") +
  theme_bw() +
  theme(axis.text=element_text(size=txs),
        #axis.text.x = element_text(hjust=1, angle=45),
        axis.title=element_text(size=txs,face="bold"),
...
```{r sus_epi_dyn, fig.width=12, fig.height=8}
res %>%
  filter(mod_name=="SLIR", type_net=="SAH Geometric", 
         alph_val%in%c(1,2,3), r%in%c(0, 0.2, 0.4, 0.6)) %>%
  rowwise() %>%
  mutate(totR=sum(m_rec, f_rec)) %>%
  ggplot(aes(x=factor(alph_val), y=totR, fill=factor(r))) + 
  geom_boxplot() + 
  facet_grid(tau~alph_type) +
  scale_fill_discrete_sequential(palette="Sunset") +
  labs(x="Strength of sex trait", y="", fill="Assortativity") +
