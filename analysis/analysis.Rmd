...
res <- read.csv("simulations-rewiring/SIR/constant_results.csv") # has different size data

# rounding r to nearest tenth (will fix in final simulations to save value instead)
res$roundR <- round(res$r, 1)
res <- filter(res, "roundR">=0)
...
#write.csv(res, "simulations-rewiring/SIR/variable_results.csv") # add R0
...
# fixing calculation of prev_ratio (max/min prev) and calculating diff_prev
outs$max_prev <- apply(outs, 1, function(x){max(x["prev_1"], x["prev_2"])})
outs$min_prev <- apply(outs, 1, function(x){min(x["prev_1"], x["prev_2"])})
outs$diff_prev <- apply(outs, 1, function(x){x["max_prev"] - x["min_prev"]})
outs$prev_ratio <- apply(outs, 1, function(x){x["max_prev"]/x["min_prev"]})
