...
res <- read.csv("simulations-rewiring/SIRS/results-combined2.csv") 
...
As expected, the relationship between assortativity  and modularity is assortativity = modularity / (1 - expected.prop.within.edges). Where expected proportion of edges within sex is equal to the 1/number of subgroups). At higher levels of assortativity, this relationship with modularity seems to break down slightly. 
...
G <- read_graph("simulations-rewiring/networks3/G_0.3N1000rep1.graphml", format="graphml")
res$roundR0 <- get_r0(res$tau, G)
```
...
res %>%
  ...
  geom_vline(xintercept=0.05, linetype="dashed", 
                color = "red", size=.75) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  facet_grid(r~roundR0) + 
  labs(x= "Proportion infected individuals", title="SIR")

```


```{r sirTrajectories, fig.height=6, fig.width=6, fig.cap="Simulated trajectories (5 replicates shown for each parameter combination) for SIRS model. Columns faceted by tau and rows faceted both by male:female susceptibility (1 and 2) and assortativity (r, range 0 to 1)."}

pvars <- expand.grid(reps=1, tau=unique(res4$tau), sigma=c(0, 0.1),
                     alph=c("1.0","2.0", "3.0"), rs=c(0, .3, .6, .9))
...
# SIR present plot
# simDf %>%
#   filter(sigma==0, alph=="1.0") %>%
#   ggplot(aes(t, I.tot, color=factor(rep))) + 
#   geom_line() + 
#   facet_grid(factor(rs) ~ tau) + 
#   labs(x="Time", y="Infecteds", title="SIR") + 
#   guides(color=FALSE)
...
res %>%
  ...
  summarise(mean_prev_ratio=mean(prev_ratio, na.rm=TRUE), 
            se=(sd(prev_ratio, na.rm=TRUE)/sqrt(n))) %>%
