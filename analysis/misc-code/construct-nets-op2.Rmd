...
res1 <- data.frame(rep=NA, degAssort=NA, clustering=NA, 
...
res2 <- data.frame(rep=NA, degAssort=NA, clustering=NA, 
                          pathLen=NA, nComponents=NA, simple=NA, giant=NA, nodes=NA,edges=NA, 
                          r=NA, q=NA)
...
```{r versionCompare, fig.cap="There are some important differences between the first and second version of the rewiring. The first version allowed for multiple components (the giant component was still close to 1000) and multiple edges (meaning self-loops) while the second-version simplified graphs (removed self-loops and multiple edges). "}

##### --- Comparison of algorithms 

compareNets <- dplyr::bind_rows(res1, res2) %>%
...
```{r version2, fig.cap="Version 2 results in a decrease in the edges in the graph because it does not allow for multiple edges. "}
