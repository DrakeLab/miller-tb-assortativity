...
Alph = [1, 1.25, 1.5, 1.75] # Ratio of male:female susceptibility
...
for x in range(0, len(var_grid)):
    for y in range(1, reps):
        ...
        yy = y % 100 + 1 # only 100 replicates of networks  (and R starts at 1)
        ...
        G = nx.read_graphml(path="networks/G_"+str(r)+"N"+str(n)+"rep"+str(yy)+".graphml")
        ...
        tots = sim.summary()
        with open("SIR/var-suscept-2/SIR_R"+str(r)+"_N"+str(n)+"_tau"+str(tau)+"_alph"+str(alph)+"_rep"+str(y)+".csv",'wb') as out:
        ...
        with open("SIR/var-suscept-2/Final_R"+str(r)+"_N"+str(n)+"_tau"+str(tau)+"_alph"+str(alph)+"_rep"+str(y)+".csv",'wb') as csv_file:
